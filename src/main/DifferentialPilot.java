 ...
 * It automatically updates a {@link lejos.robotics.localization.OdometryPoseProvider} which has called the <code>addMoveListener</code> method on
 * this object.<br>
 * Some methods optionally return immediately so the thread that called it can
 * do things while the robot is moving, such as monitor sensors and call {@link #stop()}.<br>
 ...
 *
 **/
public class DifferentialPilot implements RegulatedMotorListener, ArcRotateMoveController {
	 ...
	 */
	public DifferentialPilot(final double wheelDiameter, final double trackWidth, final RegulatedMotor leftMotor, final RegulatedMotor rightMotor) {
	 ...
	 *            running backward.
	 */
	public DifferentialPilot(final double leftWheelDiameter, final double rightWheelDiameter, final double trackWidth,
	...
	/**
	 * Stops the robot almost immediately. Use this method if the normal {@link #stop()} is too slow;
	 */
	...
	@Override
	public void arc(final double radius, final double angle, final boolean immediateReturn) {
	 ...
	 * method is similar to the {@link #arc(double radius , double angle)} method except it uses a ratio of motor speeds to determine the curvature
	 ...
	 * <p>
	 * The sign of the turn rate and the sign of the angle together determine if the robot will move forward or backward. Assuming the robot is
	 * heading toward the top of the page. Then a positive turn rate means the arc looks like this: <b> )</b> . If the angle is positive, the robot
	 * moves forward to increase its heading angle. If negative, it moves backward to decrease the heading. <br>
	 * But if the turn rate is negative, the arc looks like this: <b> ( </b> .So a positive angle (increase in heading) means the robot moves
	 * backwards, while a negative angle means the robot moves forward to decrease its heading.
	 *
	 * <p>
	 * Note: If you have specified a drift correction in the constructor it will not be applied in this method.
	 *
	 ...
	 * This method is similar to the {@link #arc(double radius, double angle, boolean immediateReturn)} method
	 ...
	 * ability to return immediately by using the <code>immediateReturn</code> parameter set to <b>true</b>.
	 *
	 * <p>
	 * The <code>turnRate</code> specifies the sharpness of the turn. Use values between -200 and +200.<br>
	 * For details about how this parameter works, see {@link #steer(double turnRate) }
	 ...
	 */
	@Override
	public synchronized void rotationStarted(RegulatedMotor motor, int tachoCount, boolean stall, long ts) { // Not used
	...
	protected void movementStart() {
		...
		for (MoveListener ml : _listeners)
			ml.moveStarted(new Move(_type, (float) _distance, (float) _angle, _robotTravelSpeed, _robotRotateSpeed, isMoving()), this);
	}
	...
	private synchronized void movementStop() {
		...
		for (MoveListener ml : _listeners)
			ml.moveStopped(new Move(_type, getMovementIncrement(), getAngleIncrement(), _robotTravelSpeed, _robotRotateSpeed, isMoving()), this);
	}
	...
	public void setLinearSpeed(double speed) {
		if (!isMoving()) {
			_robotTravelSpeed = (float) speed;
			setSpeed((int) Math.round(speed * _leftDegPerDistance), (int) Math.round(speed * _rightDegPerDistance));
		} else {
			float speedRatio = (float) speed / _robotTravelSpeed;
			_left.setSpeed(Math.round(_left.getSpeed() * speedRatio));
			_right.setSpeed(Math.round(_right.getSpeed() * speedRatio));
			_robotTravelSpeed = (float) speed;
		}
